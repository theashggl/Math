...
void main() {
  ...
  when(()=>diVariables.streamController).thenReturn(StreamController<num>.broadcast());
  setUp(() {
    gameLogic = GameLogic();
  });
  testWidgets('Material App Widget test', (WidgetTester tester) async {
      ...
      when(()=>diVariables.selectedOperation).thenReturn(SelectedOperation.divide);
      when(()=>diVariables.streamsProcessorList).thenReturn([6,2]);
      ...
      await diVariables.streamController.close();
      print('connection of stream: ${diVariables.streamController.isClosed}');
      await tester.pumpAndSettle();
